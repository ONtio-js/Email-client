...
export const syncEmailsToDatabase = async (emails: EmailMessage[], accountId: string) => {
    ...
    try {
            // await Promise.all(emails.map(email => UpsertEmail(email, accountId,index)));
...
const upsertEmailAddress  = async (address: EmailAddress, accountId: string) => {
    try {
        const existingAddress = await db.emailAddress.findUnique(
           { where:{
                accountId_address:{accountId: accountId.toString(),address: address.address?? ""}
            }
        ...
        if(existingAddress){
            return await db.emailAddress.update({
                ...
                data:{address:address.address??"", name: address.name, raw: address.raw, accountId: accountId.toString()}
            });
        }else {
...
const upsertAttachment = async (emailId: string, attachment: EmailAttachment) => {
        try {
          ...
          console.log(
            `Failed to upsert attachment for email ${emailId}: ${error}`,
          );
        }
